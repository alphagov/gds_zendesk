...
module GDSZendesk
  module TestHelpers
    def zendesk_has_no_user_with_email(email)
      stub_request(:get, "#{zendesk_endpoint}/users/search?page%5Bsize%5D=100&query=#{email}")
    ...
    def zendesk_has_user(user_details)
      stub_request(:get, "#{zendesk_endpoint}/users/search?page%5Bsize%5D=100&query=#{user_details[:email]}")
        .with(basic_auth: basic_auth_credentials)
